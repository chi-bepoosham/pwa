...
export const ShoppingCart = (props: ShoppingCartProps) => {
  const { image, material, price, size, colorCode, colorName, shop } = props;
  ...
  return (
    <div className="flex flex-col gap-4 sm:gap-6 md:gap-8 w-full items-start rounded-2xl group/pitem">
      <div className="flex flex-row justify-between items-center w-full">
        <div className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-xl overflow-hidden flex justify-center items-center group-odd/pitem:bg-[#68BAA6] group-even/pitem:bg-primary">
          <Image
            width={128}
            height={128}
            src={image}
        ...
        <div className="flex flex-col flex-grow ms-4">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm mt-2">
            <span className="text-secondary-300">{shop}</span>

            <Divider orientation="vertical" className="h-4 sm:h-5 text-secondary hidden sm:block" />

            <span className="font-semibold text-secondary-300">{size}</span>
            <Divider orientation="vertical" className="h-4 sm:h-5 hidden sm:block" />
            <div className="flex items-center gap-1">
              <span className={`text-xl sm:text-2xl text-secondary-300 ${sacramento.className}`}>
                {colorName}
              </span>
              <span
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center w-full gap-4 sm:gap-0">
        ...
        <div className="flex flex-row justify-center items-center gap-1 sm:gap-2">
          ...
          />
          <span className="text-primary-500 text-sm sm:text-base">تومان</span>
        </div>
      </div>
    </div>
  );
