...
const schema = z.object({
  phone: z
    .string({ message: 'شماره وارد شده معتبر نیست' })
...
export const LoginByPhoneOtpForm = (props: LoginByPhoneOtpFormProps) => {
  const { isLoading, submit, data, reset: resetForm } = props;

  const defaultValues = async () => {
    return data ? data : initialData;
  };

  const { handleSubmit, control, setError, setValue, resetField } =
  ...
  return (
    ...
    >
      ...
      <PhoneNumberInput control={control} changeNumber={resetForm} />
      <div className="flex flex-row justify-start items-start gap-2 mt-6">
        <InfoIcon size={20} />
        <p className="font-light text-right text-secondary">
      ...
      <div className="w-full px-8 flex justify-center">
        <Submit control={control} isLoading={isLoading} />
      </div>
    </form>
  );
};
