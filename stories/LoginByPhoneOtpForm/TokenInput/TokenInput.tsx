      ...
      render={({ field, fieldState, formState }) => (
        <div className="flex gap-1.5 justify-between">
          <InputOtp
            ...
            color="secondary"
            variant="bordered"
            classNames={{ segmentWrapper: 'flex-row-reverse justify-center outline-none ' }}
            value={field.value}
            onChange={field.onChange}
            isInvalid={fieldState.invalid}
            errorMessage={fieldState.error?.message}
            isDisabled={
              formState.isLoading || formState.isSubmitting || formState.disabled || field.disabled
            }
            onComplete={done}
            autoFocus
          />
